...
def make_H_Heisenberg_kagome_ring(num_qubits: int,
                                  endian_str: str = "big",
                                 ) -> Hamiltonian:
    """
    big endian:    q_0 = X, q_1 = Y, q_2 = Z -> q_0 q_1 q_2 = "XYZ", i.e. X \otimes Y \otimes Z
    ...
    for ith_qubit in range(num_qubits):
        ...
        ### for all qubits: linking neighbours next to each other
        observable = list("I" * num_qubits)
        observable[ith_qubit] = observable[(ith_qubit + 1) % num_qubits] = "X"
        observable = "".join(observable)[::flag_reverse_endian]
        H += {observable: 1}
        ...
        ### even index only, skipping one neighbour
        if (not (num_qubits & 1)) and (not (ith_qubit & 1)):

            observable = list("I" * num_qubits)
            observable[ith_qubit] = observable[(ith_qubit + 2) % num_qubits] = "X"
            observable = "".join(observable)[::flag_reverse_endian]
            H += {observable: 1}
            observable = list("I" * num_qubits)
            observable[ith_qubit] = observable[(ith_qubit + 2) % num_qubits] = "Y"
            observable = "".join(observable)[::flag_reverse_endian]
            H += {observable: 1}
            observable = list("I" * num_qubits)
            observable[ith_qubit] = observable[(ith_qubit + 2) % num_qubits] = "Z"
            observable = "".join(observable)[::flag_reverse_endian]
            H += {observable: 1}
...
def make_H_Heisenberg_ring_nnn(num_qubits: int,
    ...
    """
    Note that this function uses Pauli operators with \pm 1 eigenvalues, while the PRR paper uses Spins with \pm 1/2 eigenvalues
    ...
    for ith_qubit in range(num_qubits):
        ...
        ### for all qubits: linking nodes skipping one neighbour
        observable = list("I" * num_qubits)
        observable[ith_qubit] = observable[(ith_qubit + 2) % num_qubits] = "X"
        observable = "".join(observable)[::flag_reverse_endian]
        H += {observable: J2}
